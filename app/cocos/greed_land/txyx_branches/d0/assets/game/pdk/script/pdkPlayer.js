    ...
    onOutCard: function (va, isClt) {
        ...
        if (v[0] < 0) {
            ...
            err['3']={
                ...
                '-8':'不是最后一手不允许三带一或三不带',
                '-9':'不是最后一手不允许飞机不带或少带',
                '-10':'不是最后一手不允许炸弹不带或炸弹少带',
                '-11':'不是最后一手不允许连炸不带或连炸少带',
                '-12':'全是鬼牌',
                '-13':'剩余牌全是鬼牌',
            }

            let errstr = err[this.gm.roomInfo.origin];
            if (errstr && errstr[v[0]]) {
                cc.g.global.hint(errstr[v[0]]);
            } else {
                cc.g.global.hint('未知错误 '+v[0]);
            }
