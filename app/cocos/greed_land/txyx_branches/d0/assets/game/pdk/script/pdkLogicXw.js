...
cc.Class({
    ...
    */
    ...
    getCanOutCombination: function (codes, tgCodes, tgType, handCodes) {
        this.isAHC = (codes.length == handCodes.length);

        if (this.isAHC) cc.log('处理的是全手牌  考虑最后一手的可能性');
    ...
    getCodsCombination: function (codes, ctype) {
        ...
        // 三不带
        if (!ctype || (ctype==ComType.SAN) || (ctype<=ComType.SAN)) {
            // r = this.checkSan(ifo, 0);
            // if (r) {
            //     com.push(r);
            //     return com;
        ...
        if (!ctype || (ctype<=ComType.SAN)) {
            r = this.checkSan(ifo, 2);
            r && (!ctype || (r.type==ComType.SAN)) && com.push(r);
    ...
    checkSan: function (ifo, num, ctype) {
        ...
        if (res.lv) {
            ...
            (res.desc = '小炸') && (res.type = ComType.SAN);
    ...
    checkLian: function (ifo, sameNum, min, ctype) {
        ...
        let fxlk = (mlk)=>{
            ...
            if (sameNum < 3) {
                ...
                // 不是手牌全出 需要考虑带牌不足的情况
                if (!this.isAHC) {
                    if ((sct+gn) == (mlk.length * 2)) {
                        lv = mlk.length*100 + mlk[0];
                    }
                } else {
                    if ((sct+gn) <= (mlk.length * 2)) {
                        lv = mlk.length*100 + mlk[0];
        ...
        // 将连由长到短 由大到小排序
        let lenk = Object.keys(link_nl);
        lenk.sort( function(a, b) {
            return b - a;
        });
        // ==========================================
        let alllks = [];
        for (let i = 0; i<lenk.length; ++i) {
            let llks = link_nl[lenk[i]];    //长度 -- 所有连
            // 提取缩减后每条连的子链
            for (let j = 0; j < llks.length; ++j) {
                let e = llks[j];
                for (let maxl = e.length; maxl>=2; --maxl) {
                    let lk = [];
        ...
        if (res.lv) {
            if (gui > 0) {
                let mv = [];
                for (let i = 0; i < _maxlk.length; ++i) {
                    for (let j = 0; j < sameNum; ++j) {
                        mv.push(_maxlk[i]);  //mv.push(mlink[i]);
                    }
                }
    ...
    checkZD: function (ifo) {
        ...
        if (vk.length > 3) {
            cc.log('判断炸弹失败 种类超过3种');
            return null;
        } else if (vk.length+gui < 3){
            //cc.log('判断炸弹失败 带牌不够');
            //return null;
        } 

        //多牌炸弹 种类不会超过3种 因为最多带2张散牌
        let mlen = 0;
        let mlv = 0;
        for (let i = 0; i < vk.length; ++i) {
            let e = vk[i];
            ...
            let dy = tlen - len;    //去除炸弹后 多余的数量
            if (dy > 2) continue;   //带牌过多

            // 不是手牌全出 需要考虑带牌不足的情况
            if (!this.isAHC) {
                // 检查鬼牌数量 是否需要从炸弹里减少以补足带牌不够的情况
                if (gui > 0) {
                    len = len - (2-dy);
                }
            }
    ...
    getChangeFace: function (mainVals, codes, ctype) {
        ...
        {
            // 用鬼牌从2 A K 。。。。3 依次填充 不能喝主牌值及已有的牌值重复
    ...
    getMatchingCom: function (sourceCodes, mateCodes, ctype) {
        ...
        if (ctype == ComType.DAN) {
            ...
            res.forEach(e => {
                let ll = e.length/3;
                let take = this.findTake(sourceCodes, e,  ll*2, 0);
                take && addResTake(e.concat(take));
            });
    ...
    getFreeCom: function (sourceCodes) {
        ...
        if (this.gm.__shouchu) {
            ...
            for (let v = 3; v <= 15; ++v) {
                ...
                if (num==3) {
                    let take = this.findTake(sourceCodes, arr, 2, 0);
                    take && addResTake(arr.concat(take));
                    coms = coms.concat(takeres);
                } else {
                    coms.push(arr);
                }
