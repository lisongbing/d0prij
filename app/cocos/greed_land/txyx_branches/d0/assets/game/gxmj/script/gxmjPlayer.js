...
cc.Class({
    ...
    initOptFun: function () {
        ...
        // 局数
        kf[LG_opt.HaiDi.v] = this.haidi.bind(this);
        // 庄家
    ...
    dataPrepare: function () {
       ...
       this.piao = -1;
        ...
        // 胡牌断线后 可胡的牌
        this.showOpcard = null;
        if ((ri.showOpcard) && (ri.showOpcard.value>0)) {
            this.showOpcard = ri.showOpcard.value;
        }
    ...
    showNextMsg: function() {
        ...
        if (this.showOpcard != null) {
            this.gm.gameScript.curHuCode = this.showOpcard;
        }
        ...
        this.gm.gameScript.checkCanPlayMjInBaoJiao()


        let lastNum = parseInt(ri.cardNum)
        cc.log('lastNum-->' + lastNum)
        if ((lastNum == 0) && (!this.gm.xiaojjs)) {
            if ((ri.showOpcard) && (ri.showOpcard.value>0)  && (ri.showOpcard.uid.toNumber() > 0) && (ri.showOpcard.uid.toNumber() == this.d.uid.toNumber())) {
                let carNum = ri.showOpcard.value;
                let uid = ri.showOpcard.uid
                cc.log('carNum-->' + carNum)
                cc.log('uid-->' + uid)
                this.gm.gameScript.showHaiDi(parseInt(carNum), uid);
            }

            // if ((ri.opcard) && (ri.opcard.value>0) && (ri.opcard.uid.toNumber() > 0)) {
            //     let carNum = ri.opcard.value;
            //     let uid = ri.opcard.uid
            //     cc.log('carNum-->' + carNum)
            //     cc.log('uid-->' + uid)
            //     this.gm.gameScript.showHaiDi(parseInt(carNum), uid);
            // }
    ...
    canHu: function (v) {
        if (this.pg) {
            // 碰牌
            this.pg.doReciveCanHu(this.view, v);
        }
    },
    //胡
    doHu: function (v) {
    ...
    haidi: function(v) {
        if (this.pg) {
            this.pg.showHaiDi(parseInt(v[0]), v[1]);
        }
    },
    // 重置游戏
    resetPlay: function () {
