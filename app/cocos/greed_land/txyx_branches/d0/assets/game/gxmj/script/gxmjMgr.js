...
var MajhPlayer = require('gxmjPlayer');
var majhMsgQueue = require('gxmjMsgQueue');
var mjBackPlay = require('gxmjBackPlay');
let mjAudio = require('gxmjAudio');
...
cc.Class({
      ...
      let qt = [];
        for (const key in this.uidPlayers) {
            ...
            for (let j = 0; j < this.players.length; ++j) {
                let uuid = this.players[j].uid
                let eqUid = eq64(key, uuid)
                if (eqUid) {
                    isIn = true;
                    break;
                }
    ...
    prepareSettleData: function (resp) {
        ...
        //20,天十地九
        let rules = this.roomInfo.NewRlue;
        let t20 = false;
        ...
        if (!cc.g.utils.judgeArrayEmpty(allPerResult)) {
            allPerResult.forEach((item) => {
                ...
                if (item.mingtang && t20) {
                    for (let i = 0; i < item.mingtang.length; ++i) {
                        if (item.mingtang[i]==25) {
                            item.mingtang[i] = 2500;
                        }
                        if (item.mingtang[i]==26) {
                            item.mingtang[i] = 2600;
                        }
                    }
                }
            })
