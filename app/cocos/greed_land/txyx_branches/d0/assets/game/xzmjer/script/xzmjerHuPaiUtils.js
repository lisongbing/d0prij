...
var HuPaiUtils = {
	...
	setOptionRules: function(rules){
		...
		for (var i = 0;i<rules.length;i++){
			if (rules[i] == 21){
				...
				this.mingtangToFun["清一色"] = 0;
			}else if (rules[i] == 13){
				this.mingtangToFun["清一色"] = 1;
			}else if (rules[i] == 14){
				this.mingtangToFun["清一色"] = 2;
			}else if (rules[i] == 20){
				this.mingtangToFun["大对子"] = 2;
			}else if (rules[i] == 26){
				this.kaxingwu = true;
			}
		}
    ...
    showHuPaiPrompt:function(handleArr,putoutCards,que,rules){
        let result = new Map();
		this.setOptionRules(rules);
        this.back = [];
        this.result = [];
		this.qishouHandNum = 13;
		rules.forEach(h =>{
			if (h == 6){  //7张
				this.qishouHandNum = 7;
			}else if (h == 7){//10张
				this.qishouHandNum = 10;
			}else if (h == 8){//13张
				this.qishouHandNum = 13;
			}
		})
        ...
        if (handleArr.length == this.qishouHandNum){
            //七小对的情况
            let qiduiGroup = this.computeAllQiDuiGroup(handleArr,hongZhongNum);
            let hand = handleArr.slice(0,handleArr.length);
            this.computeQiDuiBack(0,qiduiGroup,hand,Math.ceil(this.qishouHandNum / 2));
    ...
    computeMaxFunQidui:function(result,putoutCards,que){
        ...
        const QIDUIROW = Math.ceil(this.qishouHandNum / 2);
        const HongZhong = 50;
        for (var i = 0; i < this.result.length/QIDUIROW; i++) {
            let hucard = this.result[QIDUIROW*i+QIDUIROW - 1][0];
    ...
    computeMaxFunNormal:function(result,putoutCards,que){
        ...
        let row = Math.ceil(this.qishouHandNum / 3) - putoutCards.length
    ...
    computQiduiFun:function(cards,putOutCards,hucard,result){
        const HongZhong = 50;
        if (hucard != HongZhong) {
            cards[Math.ceil(this.qishouHandNum / 2) - 1] = [hucard, hucard];
        } else {
            cards[Math.ceil(this.qishouHandNum / 2) -1] = [cards[Math.ceil(this.qishouHandNum / 2) - 1][0], cards[Math.ceil(this.qishouHandNum / 2) - 1][0]]
        }
    ...
    computeNotQiDuiFun:function(cards,putOutCards,hucard,result){
       ...
       if (this.yiJiuAndJiangdui){
		...
		if (this.kaxingwu){
			fun += this.computeKaXingWu(cards[cards.length - 1],hucard)
		}
        //清一色
        fun += this.computeQingYiSe(cards,putOutCards)
        //大对子
    ...
    computeJiangDui:function(cards,putOutCards,isQidui){
        let HongZhong = 50;
        if (isQidui) { //已经是七对了
            for (var i = 0; i < Math.ceil(this.qishouHandNum / 2); i++ ){
	...
	//卡星五
	computeKaXingWu:function(lastGroupCards,huCard){
		if (lastGroupCards.length == 3){
			if (lastGroupCards[0] == 24 && lastGroupCards[1] == 25 && lastGroupCards[2] == 26 && huCard == 25){
				return this.mingtangToFun["卡星五"];
			}
		}
		return 0;
	},
