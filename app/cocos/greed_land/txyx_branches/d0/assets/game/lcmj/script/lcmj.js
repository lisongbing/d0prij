...
var HuPaiUtils = require("lcmjHuPaiUtils");
var majhCtrls = require('lcmjCtrls');
var DEF = require('lcmjDef');
...
cc.Class({
    ...
    initView: function (info) {
        ...
        this.Tmp_Node_BaoZi = cc.find("Tmp_Node_BaoZi", r)
    ...
    runBaoZiAnimation: function (node) {
        ...
        node.runAction(
            cc.sequence(
                cc.delayTime(0.3),
                cc.spawn(
                    cc.moveTo(0.3, this.Tmp_Node_BaoZi.x ,this.Tmp_Node_BaoZi.y),
                    cc.scaleTo(0.25, 0.1),
                    cc.fadeTo(0.3, 255),
                ),
                cc.callFunc(
                    function (params) {
                        node.setScale(1, 1)
                        // //cc.log(node)
                        node.setPosition(this.Tmp_Node_BaoZi.x ,this.Tmp_Node_BaoZi.y);
                    },
                    self, null
                )
            )
    ...
    onButtonCopyRoom: function (event, customEventData) {
        ...
        */
        ...
        // des += cc.g.areaInfo[ri.origin].name + '隆昌麻将,';
        des += '隆昌麻将,';
        des += ri.GameNum + '局,';
        des += ri.total + '人';
    ...
    onButtonInvite: function (event, customEventData) {
        ...
        let name = gameTypes[ri.gameType] ? gameTypes[ri.gameType].NAME : '隆昌麻将';
    ...
    checkBaoJiaoCanPlayMj: function (qiPaiItemTwo) {
        let qCode = parseInt(qiPaiItemTwo.code)
        let canPlay = true
        for (let i = 0; i < this.gangCardArr.length; i++) {
            let item = this.gangCardArr[i]
            if (item.isChecked) {
                let gCode = parseInt(item.code)
                if (qCode == gCode) {
                    canPlay = false
                    break
                }
    ...
    onClickQiPaiBtnClicked: function (customData) {
        ...
        if (player.d.status == LG_Sta.CanHuan3.v) { // 换3张操作
            ...
            // 红中，则返回 或者 显示操作按钮时候 返回
            if (qiPaiItemTwo.code == 50 || qiPaiItemTwo.zezao || !canPlay) {
                return
            }
...
// 显示胡牌提示按钮
    ...
    addGangBtn: function () {
        ...
        if (!cc.g.utils.judgeArrayEmpty(codeArr)) {
            ...
            codeArr.forEach((code)=>{
                ...
                cc.g.utils.addClickEvent(htgitem, this, 'lcmj', 'onClickGangClicked', htgitem);
                // add
    ...
    addHuBtn: function () {
        ...
        cc.g.utils.addClickEvent(htgitem, this, 'lcmj', 'doClickHu', null);
        // add
        htgitemLayout.addChild(htgitem, 0, "");
        // add
        this.Node_HuTiGangView.addChild(htgitemLayout, 0, "");
    ...
    addPengFeiBtn: function (pf) {
        ...
        if (pf == 1) {
            cc.g.utils.addClickEvent(htgitem, this, 'lcmj', 'doClickFei', null);
        } else if (pf == 2) {
            cc.g.utils.addClickEvent(htgitem, this, 'lcmj', 'doClickPeng', null);
        }
        // add
        htgitemLayout.addChild(htgitem, 0, "");
        // add
    ...
    addGuoBtn: function () {
        ...
        cc.g.utils.removeClickAllEvent(htgitem);
        cc.g.utils.addClickEvent(htgitem, this, 'lcmj', 'doClickGuo', null);
        // add
    ...
    showSelectTiView: function (arr) {
        ...
        arr.forEach((code) => {
            ...
            // 添加点击事件
            cc.g.utils.addClickEvent(cardNode, this.node, 'lcmj', 'onClickTiClicked', cardNode);
            // nodejs
    ...
    showSelectGangView: function (arr) {
        ...
        arr.forEach((code) => {
            ...
            lastOneX = viewIndex * DEF.hcMultHuTiPos[0].moveBy.z + (viewIndex + 1) * (DEF.hcMultHuTiPos[0].moveBy.z / 2)
            // 添加点击事件
            cc.g.utils.addClickEvent(cardNode, this.node, 'lcmj', 'onClickGangClicked', cardNode);
            // nodejs
            this.huTi_Node_Layout.addChild(cardNode, 1, 'Node_card_Num' + viewIndex);
            // get card node
            itemArr.push(cardNode)
    ...
    doClickGuo: function () {
        if (this.checkCanPlayMj()) {
            ...
            this.gameMgr.sendOp(DEF.PlayerOpt.Guo.v, [0], this.timeStamp);
            // 点过后，重置
            this.codeLight = -100;
            this.curHuCode = 0;
            // 提示杠
    ...
    doRecivePong: function (palyerViewItem, v, uid, isRemoveCode) {
        let getRelDeskId = palyerViewItem.index; //this.gameMgr.getViewPos(deskId)
        //  显示定时器
        this.startTimer(getRelDeskId)
        ...
        // 移除弃牌
        let playerViewItemTwo = this.gameMgr.uidPlayers[uid]
        if (!cc.g.utils.judgeObjectEmpty(playerViewItemTwo) && isRemoveCode) {
            // 移除视图
            this.doRealQiPaiRemove(playerViewItemTwo.view, v)
        }
    },
    ...
    doReciveGang: function (palyerViewItem, v) {
        ...
        // cc.log('canOptType-->' + canOptType)
        // cc.log('canOptVal-->' + canOptVal)
    ...
    doReciveHu: function (palyerViewItem, v) {
        ...
        cc.log('doReciveHu v -->' + JSON.stringify(v))
    ...
    doShowBaoGangView: function(gangCardArr) {
        ...
        for(let i=0; i<gangCardArr.length; i++){
            ...
            cc.g.utils.addCheckEvent(checkBox1, this.node, 'lcmj', 'onClickBaoGangItem1', i);
            ...
            cc.g.utils.addCheckEvent(checkBox2, this.node, 'lcmj', 'onClickBaoGangItem2', i);
            ...
            this.SpBaoGang_bg_BaoJiao.height = 100 + 100*i;
        }
    ...
    //         // let isCheck = gangCardArr[i].isCheck;
    //         let checkBox1 = cc.find("Toggle_BaoGang", baogangItem1);
    //         cc.g.utils.addCheckEvent(checkBox1, this.node, 'lcmj', 'onClickBaoGangItem1', code);
    //         checkBox1.toggle = checkBox1.getComponent(cc.Toggle);
    //         checkBox1.toggle.uncheck();
    //
    //         let checkBox2 = cc.find("Toggle_BaoGang", baogangItem2);
    //         cc.g.utils.addCheckEvent(checkBox2, this.node, 'lcmj', 'onClickBaoGangItem2', code);
    //         checkBox2.toggle = checkBox2.getComponent(cc.Toggle);
    //         checkBox2.toggle.uncheck();
    //
    //         // 动态设置预制体位置
    ...
    doReComHuArr: function() {
        ...
        for (let i = 0; i < this.gangCardArr.length; i++) {
            let item = this.gangCardArr[i]
            if (item.isChecked || item.isChecked2) {
                codeArr.push(item.code)
            }
        }
